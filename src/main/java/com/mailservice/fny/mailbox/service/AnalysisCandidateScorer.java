...
import com.mailservice.fny.mailbox.service.AnalysisCandidateEvaluator.CandidateScore;
...
@Component
public class AnalysisCandidateScorer {

    CandidateScore calculateScore(
            MailAccount account,
            EmailMessage email,
            List<String> labels,
            String toHeader
        ...
        String content = normalizedContent(email);
    ...
    boolean isAutomatedNoise(List<String> labels, EmailMessage email) {
        String content = normalizedContent(email);
        String fromEmail = nullToBlank(email.getFromEmail()).toLowerCase(Locale.ROOT);
    ...
    private static String normalizedContent(EmailMessage email) {
        ...
        return (subject + " " + snippet + " " + body).toLowerCase(Locale.ROOT);
    }
